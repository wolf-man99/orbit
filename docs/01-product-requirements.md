...
Loan: ₹5,00,000 at 2% per month, `REDUCING_SIMPLE`, started 15 March, with ₹1,00,000 of principal returned on 30 April.
...
Split-cycle arithmetic of this kind is exactly what manual tracking gets wrong, and it is why the engine — not the user — owns this calculation.

> **Corrected in Phase 11.** The figures first published here (₹5,161 and ₹3,613 over 16 and 14 days) were illustrative and did not reconcile: they implied a denominator of 31 for a cycle that is 30 days long, and 16/31 + 14/31 < 1, so the cycle would never have fully accrued. The table above is generated by the implemented engine and is covered by a test. A monthly rate earns a full month per cycle; only partial stretches inside a cycle are pro-rated, by their share of that cycle's days.
